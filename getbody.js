...
if (!$.isNode() && !YouthBody == true) {
    ...
    if ($.isNode()) {
        if (process.env.YOUTH_READ && process.env.YOUTH_READ.indexOf('&') > -1) {
            ...
            var fs = require("fs");
            YouthBodys = fs.readFileSync("zq_body.txt", "utf8");
            if (YouthBodys !== `undefined`) {
            YouthBodys = YouthBodys.split("\n");}
       ...
       if($request.url.match(/complete/)){
        ...
        if (YouthBody) {
            if (YouthBody.indexOf(bodyVal) > -1) {
				...
				var fs = require("fs");
				fs.wirteFileSync("zq_time.txt",bodyVal);
				$.log(`${$.name}写body: 成功, YouthBodys: ${bodyVal}`);
